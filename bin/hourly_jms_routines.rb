...
res = []
topics.each  { |t|  cmd = "`jruby -S jms_message_send.rb -T #{t} -M stomp_PING -b nil -A true`" 
                    puts "About to run #{cmd}"
                      temp = eval(cmd)
                      res << "TOPIC #{t}: " + temp if !temp.include?("ALIVE")
                    }
                      StompMessage::StompSendTopic.send_email_stomp("scott.sproule@cure.com.ph","STOMP ALERT", 'scott.sproule@cure.com.ph',  "problems: [#{res.size}] with hourly jms", res.to_s) if  res.size > 0
